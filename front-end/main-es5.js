...
var AppComponent = /** @class */ (function () {
    ...
    AppComponent.prototype.getBackgroundColor = function () {
        if (this.showStyle) {
            return 'yellow';
        }
        else {
    ...
    AppComponent.prototype.setMyStyles = function () {
        var styles = {
    ...
    AppComponent.prototype.getClassByValue = function (value) {
        switch (value) {
...
var ColumnSorterComponent = /** @class */ (function () {
    function ColumnSorterComponent(elementRef, columnSorterService) {
    ...
    ColumnSorterComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.columnInfo = this.columns.map(function (currElement, index) {
            return {
                id: currElement,
                name: _this.columnNames[index],
                hidden: false,
            };
        });
    ...
    ColumnSorterComponent.prototype.ngAfterViewInit = function () {
        this.elementRef.nativeElement.classList += 'va-mat-button-no-input';
    };
    ColumnSorterComponent.prototype.columnMenuDropped = function (event) {
    ...
    ColumnSorterComponent.prototype.toggleSelectedColumn = function (columnId) {
        var colFound = this.columnInfo.find(function (col) { return col.id === columnId; });
        colFound.hidden = !colFound.hidden;
        this.emitColumns(true);
    };
    ColumnSorterComponent.prototype.emitColumns = function (saveColumns) {
        var _this = this;
        // Only emit the columns on the next animation frame available
        window.requestAnimationFrame(function () {
            _this.columnsChange.emit(_this.columnInfo.filter(function (colInfo) { return !colInfo.hidden; }).map(function (colInfo) { return colInfo.id; }));
            if (saveColumns) {
                _this.columnSorterService.saveColumnInfo(_this.columnInfo, _this.saveName);
            }
        });
    };
    ColumnSorterComponent.prototype.openDialog = function () {
    };
    ColumnSorterComponent.ctorParameters = function () { return [
...
var ColumnSorterService = /** @class */ (function () {
    function ColumnSorterService() {
    }
    ColumnSorterService.prototype.loadSavedColumnInfo = function (columnInfo, saveName) {
    ...
    ColumnSorterService.prototype.saveColumnInfo = function (columnInfo, saveName) {
...
var DataTableComponent = /** @class */ (function () {
    function DataTableComponent(tableService, fb, zone, ref) {
    ...
    Object.defineProperty(DataTableComponent.prototype, "fromDate", {
        // getter for the date picker forms
        get: function () { return this.filterForm.get('fromDate').value; },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(DataTableComponent.prototype, "toDate", {
        get: function () { return this.filterForm.get('toDate').value; },
        enumerable: true,
        configurable: true
    });
    // on row click preview table data
    DataTableComponent.prototype.onRowClicked = function (element) {
    ...
    // search filter
    DataTableComponent.prototype.applySearchFilter = function (event) {
    ...
    // date filter method
    DataTableComponent.prototype.applyDateFilter = function (value) {
        ...
        this.dataSource.data = this.dataSource.data.filter(function (item) {
            ...
            var dateA = new Date(a.Shipdate);
            var dateB = new Date(b.Shipdate);
            return dateA.valueOf() - dateB.valueOf();
        });
    };
    // submit order note when making new note or editing
    DataTableComponent.prototype.onSubmit = function () {
        var _this = this;
        this.tableService.addJobData(this.tableDataForm.value).subscribe(function () {
            _this.loadData();
        });
    };
    // trigger the edit capability
    DataTableComponent.prototype.onEdit = function () {
        this.isEditEnable = !this.isEditEnable;
    };
    // load data from the http client
    DataTableComponent.prototype.loadData = function () {
        var _this = this;
        this.tableService.getJobData().subscribe(function (data) {
            _this.dataSource._data.next(data);
            _this.dataSubject.next(data);
            _this.ref.detectChanges();
        });
        // Pass the  table data to material table data source
        this.tableData$.pipe().subscribe(function (res) {
            _this.dataSource = new _angular_material__WEBPACK_IMPORTED_MODULE_3__["MatTableDataSource"](res);
            _this.dataSource.paginator = _this.paginator;
        });
        this.tableService.data$.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["map"])(function (res) { return res.tableCount; }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["tap"])(function (res) { return _this.tableCount = res; })).subscribe();
    };
    // change cell color based on number of times the property count exists
    DataTableComponent.prototype.getClassByValue = function (a) {
        for (var j = 0; j < this.tableCount.length; j++) {
            for (var property in this.tableCount[j]) {
                if (property === a) {
    ...
    DataTableComponent.prototype.countCheck = function (value) {
        if (value > 10) {
            return 'class-one';
        }
        switch (value) {
                ...
                for (var i = 0; i < nodeArray_1.length; i++) {
                    if (nodeArray_1[i].nodeName === 'BUTTON') {
                        if (nodeArray_1[i].innerHTML.length > 100 && nodeArray_1[i].disabled) {
                            _this.ren.setStyle(nodeArray_1[i], 'background-color', '#0257FF');
                            _this.ren.setStyle(nodeArray_1[i], 'color', 'white');
                            ...
                            _this.ren.setStyle(nodeArray_1[i], 'background-color', 'lightgray');
                        }
                    }
                }
            });
    ...
    StylePaginatorDirective.prototype.createButton = function (i, pageIndex) {
        ...
        switch (i) {
            ...
            case this._pageGapTxt:
                var newIndex_1 = this._curPageObj.pageIndex + this._showTotalPages;
                if (newIndex_1 >= this.numOfPages) {
                    newIndex_1 = this.lastPageIndex;
                }
                if (pageIndex != this.lastPageIndex) {
                    this.ren.listen(linkBtn, 'click', function () {
                        console.log('working: ', pageIndex);
                        _this.switchPage(newIndex_1);
                    });
                }
                if (pageIndex == this.lastPageIndex) {
                    this.ren.setAttribute(linkBtn, 'disabled', 'disabled');
...
var MatDataTableComponent = /** @class */ (function () {
    function MatDataTableComponent() {
    }
    MatDataTableComponent.prototype.ngOnInit = function () {
    };
...
var TableService = /** @class */ (function () {
    ...
    // });
    TableService.prototype.addJobData = function (element) {
        console.log(element);
        // const url = `${this.jobUrl}/api/submit-data`;
        var url = "/api/submit-data";
        return this.http.post(url, element).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(function (res) { return console.log(res); }));
    };
    TableService.prototype.findLessons = function (filter, sortOrder, pageNumber, pageSize) {
        if (filter === void 0) { filter = ''; }
        if (sortOrder === void 0) { sortOrder = 'asc'; }
        if (pageNumber === void 0) { pageNumber = 0; }
        if (pageSize === void 0) { pageSize = 3; }
        var url = this.jobUrl + "/api/get-all-table-data";
        ...
        }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(function (res) { return res['data']; }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(function (res) { return console.log(res); }));
    };
